...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
      //Moved coce out of the for-loop
      var dx = determineDx(document.querySelectorAll(".randomPizzaContainer")[i], size);
      var newwidth = (document.querySelectorAll(".randomPizzaContainer")[i].offsetWidth + dx) + 'px';
      var pizzaCount = document.querySelectorAll(".randomPizzaContainer").length;

      for (var i = 0; i < pizzaCount; i++) {
      //Changed from the slower document.querySelectorAll(".randomPizzaContainer")[i].style.width = newwidth;
...
// Eliminating the need for retrieving same element over and over again.
...
function updatePositions() {
  ...
  //Removed the following code from the for loop. No need to calculate it every time.
